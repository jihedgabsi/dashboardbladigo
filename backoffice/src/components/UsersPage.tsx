...
const UsersPage: React.FC<UsersPageProps> = ({ onNavigate }) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/api/user/alldashboardusers', {
        method: 'GET',
        headers: {
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50 transition-colors duration-200">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs rounded-full ${getRoleColor(user.role)}`}>Client</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-96 max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-800">Détails de l'utilisateur</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div className="bg-gray-50 p-3 rounded-md">
                  ...
                  <span className={`px-2 py-1 text-xs rounded-full ${getRoleColor(viewingUser.role)}`}>Client</span>
                </div>
                <div className="bg-gray-50 p-3 rounded-md">
